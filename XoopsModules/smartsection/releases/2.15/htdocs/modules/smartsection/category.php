...
if ($GLOBALS['xoopsModuleConfig']['htaccess']&&strpos($_SERVER['REQUEST_URI'], 'odules/'))
{
	...
	if ($categoryid<>0)
		header('Location: '.XOOPS_URL.'/'.$GLOBALS['xoopsModuleConfig']['basepath'].'/'.xoops_sef($categoryObj->getVar('name')).'/'.$categoryid.$GLOBALS['xoopsModuleConfig']['endofurl_html']);
	else
		header('Location: '.XOOPS_URL.'/'.$GLOBALS['xoopsModuleConfig']['basepath'].'/cat,0'.$GLOBALS['xoopsModuleConfig']['endofurl_html']);
	exit(0);
}
		...
		if (isset($subcatsObj)) {
			foreach ($subcatsObj as $key => $subcat) {
				...
				if (($subcat_total_items > 0) || ($xoopsModuleConfig['show_subcats'] == 'all' )) {
					...
					// Put this subcat in the smarty variable
					$subcategories[$key] = $subcat->toArray();
					$subcategories[$key]['name'] = htmlspecialchars_decode($subcategories[$key]['name']);
					//$total += $numItems;
				}
			}
		}
