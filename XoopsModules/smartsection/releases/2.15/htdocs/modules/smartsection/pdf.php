...
if ($GLOBALS['xoopsModuleConfig']['htaccess']&&strpos($_SERVER['REQUEST_URI'], 'odules/'))
{
	...
	header('Location: '.XOOPS_URL.'/'.$GLOBALS['xoopsModuleConfig']['basepath'].'/'.xoops_sef($categoryObj->getVar('name')).'/'.xoops_sef($itemObj->getVar('title')).'/pdf,'.$itemid.','.$itemObj->categoryid().$GLOBALS['xoopsModuleConfig']['endofurl_pdf']);
	exit(0);
}
